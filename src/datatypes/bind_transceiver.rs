...
impl BindTransceiver {
    ...
    pub fn validate(&self) -> Result<(), BindTransceiverValidationError> {
        ...
        if self.system_id.len() > MAX_SYSTEM_ID_LENGTH {
            return Err(BindTransceiverValidationError::SystemIdTooLong {
        ...
        if let Some(ref password) = self.password {
            if password.len() > MAX_PASSWORD_LENGTH {
                return Err(BindTransceiverValidationError::PasswordTooLong {
                    actual: password.len(),
                });
            }
        }
        ...
        if self.system_type.len() > MAX_SYSTEM_TYPE_LENGTH {
            return Err(BindTransceiverValidationError::SystemTypeTooLong {
                actual: self.system_type.len(),
            });
        }

        if self.address_range.len() > MAX_ADDRESS_RANGE_LENGTH {
            return Err(BindTransceiverValidationError::AddressRangeTooLong {
...
impl ToBytes for BindTransceiver {
    fn to_bytes(&self) -> Bytes {
        ...
        self.validate().expect("BindTransceiver validation failed");
...
mod tests {
    ...
    fn bind_transceiver_builder_basic() {
        ...
        assert_eq!(bind_transceiver.system_id, "TEST");
        assert_eq!(bind_transceiver.password, Some("secret".to_string()));
        assert_eq!(bind_transceiver.system_type, "VMS");
        assert_eq!(bind_transceiver.address_range, "1234");
        assert_eq!(
    ...
    fn bind_transceiver_validation_system_id_too_long() {
        ...
        let validation_result = bind_transceiver.validate();
        assert!(validation_result.is_err());
        assert!(matches!(
            validation_result.unwrap_err(),
            BindTransceiverValidationError::SystemIdTooLong { .. }
        ));
    }
