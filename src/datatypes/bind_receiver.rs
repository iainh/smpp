...
mod tests {
    ...
    fn bind_receiver_to_bytes() {
        ...
        let br_bytes = Encodable::to_bytes(&bind_receiver);

        // Expected byte representation of a bind receiver (SMPP v3.4 fixed-length)
    ...
    fn bind_receiver_response_to_bytes() {
        ...
        // Expected byte representation of a bind receiver response without TLV (SMPP v3.4 fixed-length)
        let expected: Vec<u8> = vec![
            // Header:
            0x00, 0x00, 0x00,
